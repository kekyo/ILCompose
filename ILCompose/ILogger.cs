...
namespace ILCompose
{
    ...
    public abstract class LoggerBase : ILogger
    {
            ...
            string? header, LogLevels logLevel, FormattableString? message, Exception? ex)
        {
            ...
            {
                case ({ }, { }):
                    return $"ILCompose [{ThisAssembly.AssemblyVersion}]{GetHeaderString(header)}:{GetLogLevelString(logLevel)} {message}, {ex}";
                case ({ }, null):
                    return $"ILCompose [{ThisAssembly.AssemblyVersion}]{GetHeaderString(header)}:{GetLogLevelString(logLevel)} {message}";
                case (null, { }):
                    return $"ILCompose [{ThisAssembly.AssemblyVersion}]{GetHeaderString(header)}:{GetLogLevelString(logLevel)} {ex}";
                default:
                    return null;
            }
        }
